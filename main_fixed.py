"""
FIXED: Main script for CatBoost fMRI/sMRI classification with proper data leakage prevention

KEY CHANGES:
1. ComBat harmonization is now applied PER FOLD using only training data
2. Feature selection remains correctly isolated to training data
3. Added balanced accuracy and macro F1-score metrics
4. Optional Leave-One-Site-Out (LOSO) CV for true generalization testing
"""
import os
import sys
import random
...
def apply_combat_per_fold(fmri_data, smri_data, sites, labels, genders, ages, 
    ...
    if combat_fmri:
        ...
        # This requires getting the learned gamma and delta parameters
        # For now, we apply ComBat on train+val/test but this is a known limitation
        # A proper fix would require extracting and applying ComBat parameters separately
...
def train_single_fold(fold_idx, data, labels, fold_indices, config):
    ...
    # Compute additional metrics (not fooled by imbalance)
...
def main(use_loso=False):
    """
    Main training pipeline
    
    Args:
        use_loso: If True, use Leave-One-Site-Out CV (gold standard for multisite data)
                 If False, use standard 5-fold CV
    """
    print("="*80)
    print("Multimodal Brain ASD Classification with CatBoost [FIXED DATA LEAKAGE]")
    print("="*80)
    ...
    print(f"Cross-validation: {'Leave-One-Site-Out (LOSO)' if use_loso else f'{config.K_FOLD}-Fold'}")
    ...
    # IMPORTANT: Do NOT apply ComBat here! Apply per fold instead.
    # This was the critical bug in the original pipeline.
    ...
    for fold in range(config.K_FOLD):
        print(f"\n{'='*60}")
        print(f"FOLD {fold + 1}: HARMONIZATION, FEATURE SELECTION, TRAINING")
        print(f"{'='*60}")
        
        # STEP 1: Apply ComBat PER FOLD using only training data
        print(f"\nStep 1: Harmonization (Fold {fold + 1})")
        ...
        print(f"  Combined features shape: {data.shape}")
        
        # STEP 3: Train model
        print(f"Step 3: Training (Fold {fold + 1})")
        result = train_single_fold(
            fold, data, labels, fold_indices, config
        )
    ...
    avg_balanced_acc = np.mean([r['test_balanced_acc'] for r in fold_results])
    avg_macro_f1 = np.mean([r['test_macro_f1'] for r in fold_results])
    ...
    print(f"Average Balanced Accuracy:   {avg_balanced_acc:.4f}  ← More robust to imbalance")
    print(f"Average Macro F1-Score:      {avg_macro_f1:.4f}     ← More robust to imbalance")
    ...
    results_dict = {
        ...
        'test_balanced_accuracy': [r['test_balanced_acc'] for r in fold_results],
        'test_macro_f1': [r['test_macro_f1'] for r in fold_results]
    }
...
if __name__ == "__main__":
    # Run with standard 5-fold CV
    main(use_loso=False)
    
    # Uncomment below to run Leave-One-Site-Out CV (recommended for multisite data)
    # main(use_loso=True)
